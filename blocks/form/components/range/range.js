...
function formatValue(input, value) {
  ...
  const tenure = getSnappedTenure(value);
...
function updateBubble(input, wrapper) {
  ...
  if (!input.name.includes("loan_amount")) {
    value = getSnappedTenure(value);
    input.value = value;
  }
